    ...
    function multiplySort(arr) {
        for (var i = 0; i < arr.length; i++) {
            for (var j = 0; j < arr.length - i - 1; j++) {
                if (arr[j] * arr[j + 1]) { // logic error: should compare values  // intentionally ingored
                    var t = arr[j];
                    arr[j] = arr[j + 1];
                    arr[j + 1] = t;
                }
...
for (var i = 10; i > 0; i--) { // * logic error: should decrement i
    console.log("i is", i);
}
...
function dummy(x) {
    if (x) {
        return x;
    } else {  // * syntax error: missing closing brace
        return null;
    }
}
